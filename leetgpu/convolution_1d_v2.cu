...
__constant__ float kernel_func[2048];

constexpr unsigned TN = 256;
constexpr unsigned TILE = 256;

__global__ void convolution_1d_kernel(const float* input, float* output, int input_size, int kernel_size, int output_size) {
    ...
    for (int k = 0; k < kernel_size; k += TILE) {
        ...
        if (gx < output_size) {
            ...
            for (int i = 0; i < current_tile_w; i++) {
                sum += ssm_data[tx + i] * kernel_func[k + i];
            }
        }
        __syncthreads();
    }
...
extern "C" void solve(const float* input, const float* kernel, float* output, int input_size, int kernel_size) {
    ...
    cudaMemcpyToSymbol(kernel_func, kernel, sizeof(float) * kernel_size);
    convolution_1d_kernel<<<blocksPerGrid, threadsPerBlock>>>(input, output, input_size, kernel_size, output_size);
